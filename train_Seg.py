...
parser.add_argument('--crop_x', type=int, default=256,
                    help='Horizontal random crop size')
parser.add_argument('--crop_y', type=int, default=256,
                    help='Vertical random crop size')
parser.add_argument('--alpha', type=float, default=1.0,
                    help='Coefficient for classification loss term')
parser.add_argument('--start-lr', type=float,
                    default=0.001, help='Learning rate')
parser.add_argument('--milestones', type=str,
                    default='10,20,30', help='Milestones for LR decreasing')
...
if __name__ == '__main__':
    ...
    for epoch in range(1+starting_epoch, 1+starting_epoch+args.epochs):
        ...
        for count, (x, y, y_cls) in enumerate(train_loader):
            ...
            # forward
            out, out_cls = net(x)
            seg_loss, cls_loss = seg_criterion(
                out, y), cls_criterion(out_cls, y_cls)
        ...
        if epoch % 10 == 0:
            torch.save(net.state_dict(), os.path.join(
                models_path, '_'.join(["PSPNet", str(epoch)])))

    torch.save(net.state_dict(), os.path.join(
        models_path, '_'.join(["PSPNet", 'last'])))
