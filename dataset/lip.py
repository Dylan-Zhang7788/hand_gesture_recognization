...
class LIP(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, gt_transform=None):
        ...
        if self.train:
            self.train_image_path, self.train_gt_path = self.read_labeled_image_list(
                osp.join(root, 'train'))
        else:
            self.val_image_path, self.val_gt_path = self.read_labeled_image_list(
                osp.join(root, 'val'))
            # self.test_image_path = self.read_labeled_image_list(osp.join(root, 'test'))

    def __getitem__(self, index):
        if self.train:
            img, gt = self.get_a_sample(
                self.train_image_path, self.train_gt_path, index)
        else:
            img, gt = self.get_a_sample(
                self.val_image_path, self.val_gt_path, index)
        return img, gt
    ...
    def get_a_sample(self, image_path, gt_path, index):
        # get PIL Image
        # .resize((512,512),resample=Image.BICUBIC)
        img = Image.open(image_path[index])
        if len(img.getbands()) != 3:
            img = img.convert('RGB')
        # .resize((30,30),resample=Image.NEAREST)
    ...
    def read_labeled_image_list(self, data_dir):
        ...
        f = open('./dataset/train_Seg/train/id.txt', 'r')
        image_path = []
        gt_path = []
        for line in f:
            image = line.strip("\n")
...
class LIPWithClass(LIP):
    ...
    def __getitem__(self, index):
        if self.train:
            img, gt, gt_cls = self.get_a_sample(
                self.train_image_path, self.train_gt_path, index)
        else:
            img, gt, gt_cls = self.get_a_sample(
                self.val_image_path, self.val_gt_path, index)
    ...
    def get_a_sample(self, image_path, gt_path, index):
        ...
        # 统计每一个类别的个数
        gt_cls, _ = np.histogram(
            gt_np, bins=self.num_cls, range=(-0.5, self.num_cls-0.5), )
...
if __name__ == "__main__":
    ...
    transform_gt_list = [
        ...
        transforms.Lambda(lambda image: Image.fromarray(
            np.uint8(np.asarray(image)*(255.0/19.0)))),
        transforms.ToTensor(),
    ]
    ...
    for count, (src, lab) in enumerate(loader):
        ...
        src = denormalize(src, [0.485, 0.456, 0.406], [
                          0.229, 0.224, 0.225]).transpose(1, 2, 0)
